...
class TestGetIpfsFileHash:
    ...
    def test_valid_cid_string_bytes_returns_cid(self):
        """When the bytes decode to a valid CID string, return that CID's str()."""
        fake_cid = MagicMock()
        fake_cid.__str__ = MagicMock(return_value="bafytest123")
        with patch(
            "packages.valory.skills.task_execution.utils.ipfs.CID"
        ) as mock_cid_cls:
            mock_cid_cls.from_string.return_value = fake_cid
            result = get_ipfs_file_hash(b"bafybeigdyrzt5sfp7udm7hu76uh7y26nf3efuylqabf3oclgtqy55fbzdi")
        assert result == "bafytest123"

    def test_fallback_to_sha256_on_invalid_bytes(self):
        ...
        fake_fallback_cid.__str__ = MagicMock(return_value="f_fallback_cid")
        with patch(
            "packages.valory.skills.task_execution.utils.ipfs.CID"
        ) as mock_cid_cls:
    ...
    def test_cid_prefix_constant(self):
...
class TestToMultihash:
    ...
    def test_strips_first_six_hex_chars(self):
        """Verify the function returns hex of multihash bytes with first 6 chars stripped."""
        multihash_bytes = bytes.fromhex("1220" + "ab" * 32)  # sha2-256 multihash prefix + 32 bytes
        decoded_with_prefix = b"\x01\x70" + multihash_bytes  # CID v1 + dag-pb codec

        mock_cid_str = "bafytesthash"
        with (
            patch("packages.valory.skills.task_execution.utils.ipfs.multibase") as mock_mb,
            patch("packages.valory.skills.task_execution.utils.ipfs.multicodec") as mock_mc,
        ):
    ...
    def test_calls_multibase_decode(self):
...
class TestComponentPackageLoader:
    ...
    def test_valid_package_returns_tuple(self):
        """Valid package returns (component_yaml_dict, entry_point_source, callable_name)."""
        component_yaml, entry_point, callable_method = ComponentPackageLoader.load(VALID_PACKAGE)
