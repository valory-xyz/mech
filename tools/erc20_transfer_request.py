...
The agent already has/knows the owned address for the agent service, "my_address". The agent does not know the receiving address, “recipient_address” or the value, “value” 
the user prompt indicates to send. The rest of the transaction object that is not known beforehand must be constructed by you from the user's prompt information. 
...
def erc20_transfer(**kwargs):
    """
    Execute an erc20 token transfer on Ethereum. This involves:
    ...
    4. signing the transaction object with the private key of the agent 
    5. sending the signed transaction object + other data if desired to the agent process
    """
