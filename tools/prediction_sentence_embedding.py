...
has only two possible outcomes: `Yes` or `No`. Each market has a closing date at which the outcome is evaluated. This date is typically stated within the market question.  \
The closing date is considered to be 23:59:59 of the date provided in the market question. All market questions ask for the status of an event or a process at a specific point in time. \
The following fictional 'EXAMPLES' demonstrate the evaluation of the outcome of a market question, given its phrasing.

EXAMPLES:
* Example 1: 
    - Market question: "Will Donald Trump continue to be the president of the United States on 12 March 2022?"
    - Implied status: Donald Trump is currently the president of the United States.
    - Question asks if the status will NOT change by 12 March 2022.
    - If status changes by 12 March 2022, the market question's outcome is `No`.
    - If status does not change by 12 March 2022, the market question's outcome is `Yes`.
    - If status has changed recently, and will not change again by 12 March 2022 the market question's outcome is `No`.
    - If status will change after 12 March 2022, the market question's outcome is `Yes`.

* Example 2: 
    - Market question: "Will Colorado break its record for the hottest day by 19 November 2019?"
    - Implied status: Colorado has not broken its record for the hottest day recently.
    - Question asks if the status will change by 19 November 2019.
    - If status changes by 19 November 2019, the market question's outcome is `Yes`.
    - If status does not change by 19 November 2019, the market question's outcome is `No`.
    - If status has changed recently, the market question's outcome is `Yes`.
    - If status will change after 19 November 2019, the market question's outcome is `No`.

* Example 3: 
    - Market question: "Will Microsoft's new AI assistant become available to all by 11 July 2015?"
    - Implied status: Microsoft's new AI assistant is currently not available to all.
    - Question asks if the status will change by 11 July 2015.
    - If status changes by 11 July 2015, the market question's outcome is `Yes`.
    - If status does not change by 11 July 2015, the market question's outcome is `No`.
    - If status has changed recently, the market question's outcome is `Yes`.
    - If status will change after 11 July 2015, the market question's outcome is `No`.

* Example 4:
    - Market question: "Will Taylor Swift release a new album on 12 January 2022?"
    - Implied status: Taylor Swift has not released a new album recently.
    - Question asks if the status will change exactly on 12 January 2022.
    - If status changes exactly on 12 January 2022, the market question's outcome is `Yes`.
    - If status does not change exactly on 12 January 2022, the market question's outcome is `No`.
    - If status has changed recently, and will not change again on 12 January 2022, the market question's outcome is `No`.
    - If status will change after 12 January 2022, the market question's outcome is `No`.

* Example 5:
    - Market question: "Will Amazon launch a second internet satellite on or before 24 October 2023?"
    - Implied status: Amazon has launched exactly one internet satellite.
    - Question asks if the status will change on or before 24 October 2023.
    - If status changes on or before 24 October 2023, the market question's outcome is `Yes`.
    - If status does not change on or before 24 October 2023, the market question's outcome is `No`.
    - If status has changed recently, the market question's outcome is `Yes`.
    - If status will change after 24 October 2023, the market question's outcome is `No`.
...
* Evaluate the 'market question' and determine the implied status as well as what the question asks for.
...
* The probability estimations must be as accurate as possible, as an inaccurate estimations will lead to financial loss for the user.
* Examine the itemized list under "ADDITIONAL_INFORMATION" thoroughly. This data is sourced from a Google search engine query done a few seconds ago. 
* Use any item in "ADDITIONAL_INFORMATION" that is relevant for the probability estimation in addition to your training data. You can assume that you have been provided with the most current information available on the internet. Still pay close attention on the release and modification timestamps provided in parentheses right before each information item. Some information might be outdated and not relevant anymore.
* Use the examples provided under "EXAMPLES" as a reference to unserstand how to interpret the 'market question' and how to use the information provided under "ADDITIONAL_INFORMATION".
* More recent information within "ADDITIONAL_INFORMATION" indicated by the timestamps provided in parentheses right before each information item overrides older information within "ADDITIONAL_INFORMATION" and holds more weight for your probability estimation.
...
* Use the curret time, which is `{timestamp}`, for your probability estimation and incorporate the remaining time until the closing date of the 'market question' into your probability estimation.
